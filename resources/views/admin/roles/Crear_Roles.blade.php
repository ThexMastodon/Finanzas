...
@section('content')
  <div class="container-fluid">
    <div class="row">
      <div class="col-md-12">
        <div class="card">
          <form id="form-nuevo-rol" action="{{ route('Crear-Roles') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="card-body">
              <h4>{{ __('Crear un nuevo rol') }}</h4>
              <div class="row">
                <div class="col-md-6">
                  <div class="form-group">
                    <label for="nombreInput">Nombre del Nuevo Rol</label>
                  ...
                  <div class="card card-primary card-outline">
                    <div class="card-body box" id="permisos-container">
                      ...
                      @foreach ($permisos2 as $permiso)
                        <div class="form-check">
                          <input class="form-check-input" type="checkbox" name="permisos2[]" value="{{ $permiso->id }}"
                            id="permiso{{ $permiso->id }}">
                          <label class="form-check-label" for="permiso{{ $permiso->id }}">{{ $permiso->name }}</label>
                        </div>
                      @endforeach
                      ...
                      @foreach ($permisos3 as $permiso)
                        <div class="form-check">
                          <input class="form-check-input" type="checkbox" name="permisos3[]" value="{{ $permiso->id }}"
                            id="permiso{{ $permiso->id }}">
                          <label class="form-check-label" for="permiso{{ $permiso->id }}">{{ $permiso->name }}</label>
                        </div>
                      @endforeach
  ...
  <script type="text/javascript">
    $(document).ready(function() {
      ...
      document.getElementById("btn-actualizar").addEventListener("click", function(event) {
        ...
        form = $("#form-nuevo-rol");
        var form_data = new FormData($('#form-nuevo-rol')[0]);
        $.ajax({
